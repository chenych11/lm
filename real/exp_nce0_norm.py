...
from models import LangModel, Graph, optimizers, categorical_crossentropy, objective_fnc, slice_X, \
    np, theano, TableSampler, Split, containers, T, LookupProb, logger, Embedding, math, make_batches, \
    PartialSoftmax, Dense, LogInfo, MAX_SETN_LEN, grouped_sentences, time, chunk_sentences, LangLSTMLayer
from layers import ActivationLayer


class NCELangModelV0(Graph, LangModel):
    def __init__(self, vocab_size, nb_negative, embed_dims=128, context_dims=128,
                 negprob_table=None, optimizer='adam'):
        super(NCELangModelV0, self).__init__(weighted_inputs=False)
    ...
    def compile(self):
        ...
        def __summarize_outputs(outs, batch_sizes):
            ...
            loss, encode_len, nb_words, sum_unrm, squre_urm = out
            ...
            smry_unrm = sum_unrm.sum()
            smry_sq_unrm = squre_urm.sum()
            return [smry_loss, smry_encode_len, smry_nb_words, smry_unrm, smry_sq_unrm]
...
model = NCELangModelV0(vocab_size=nb_vocab, nb_negative=options.negative,
