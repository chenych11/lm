...
python_command="python main_nce7.py"
...
for ((nb_vocab=10000; nb_vocab<30000; nb_vocab+=2000)); do
    log_file="../logs/main-nce7-C${context_size}-E${embed_size}-lr${lr}-lr_min${lr_min}-g${gamma}-V${nb_vocab}-N${nb_neg}.log"
...
#nb_neg=50
#nb_vocab=30000
#for lr in 0.07 0.03 0.02 0.01 0.008 0.006; do
#    log_file="../logs/main-nce7-C${context_size}-E${embed_size}-lr${lr}-lr_min${lr_min}-g${gamma}-V${nb_vocab}-N${nb_neg}.log"
...
#    log_file="../logs/main-nce7-C${context_size}-E${embed_size}-lr${lr}-lr_min${lr_min}-g${gamma}-N${nb_neg}.log"
#    $command_prefix $python_command -C ${context_size} -E ${embed_size} \
#        --lr=${lr} --lr-min=${lr_min} \
#        -d --gamma=${gamma} -N ${nb_neg} \
#        -S $coding_file -e $embed_file --log-file $log_file \
