...
from models import LBLangModelV2, logger
...
parser.add_option("-w", "--nb-workers", type="int", dest="nb_workers", default=3,
...
if embedding_file != '':
    ...
    embed = compose_dense_repr(nb_base, nb_vocab, freq, embedding_file)
    embed = np.vstack([embed, np.zeros((options.context_size, options.embed_size))])
    ini_embeds = [embed]
else:
    ini_embeds = None
...
model.train(data_file=options.corpus,
            ...
            log_file=log_file,
            nb_data_workers=options.nb_workers)